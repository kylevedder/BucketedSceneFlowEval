...
class QueryParticleLookup:
    ...
    def __init__(self, num_entries: int, query_init_timestamp: Timestamp):
        ...
        self.query_init_world_particles = np.zeros((num_entries, 3), dtype=np.float32)
    ...
    def __getitem__(self, particle_id: ParticleID) -> Tuple[WorldParticle, Timestamp]:
        ...
        return self.query_init_world_particles[particle_id], self.query_init_timestamp

    def __setitem__(self, particle_id: ParticleID, value: WorldParticle):
        assert (
            particle_id < self.num_entries
        ).all(), f"particle_ids {particle_id[particle_id >= self.num_entries]} must be less than {self.num_entries}"
...
class QuerySceneSequence:
    ...
    def __init__(
        ...
        assert isinstance(
            query_trajectory_timestamps, list
        ...
        assert set(query_trajectory_timestamps).issubset(
