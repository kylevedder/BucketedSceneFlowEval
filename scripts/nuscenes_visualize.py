...
from bucketed_scene_flow_eval.datasets.nuscenes.nuscenes_raw_data import NuScenesLoader
...
if __name__ == "__main__":
    ...
    nuscenes = NuScenesLoader("/tmp/nuscenes/", version="v1.0-mini", verbose=True)
    print("All of the scene tokens", len(nuscenes))
    for i, scene in enumerate(nuscenes):
        print(f"All of the synced frames for scene: {i}", len(scene))

    first_scene = nuscenes[0]
    raw_frames = [first_scene.load(i, i-1)[0] for i in range(len(first_scene))]

    # print("Loading sequence idx", vis_index)
    # frame_list = dataset[vis_index]
    visualize_lidar_3d(raw_frames, 2)
