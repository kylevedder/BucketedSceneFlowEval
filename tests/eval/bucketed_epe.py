...
# from bucketed_scene_flow_eval.datasets.shared_datastructures
...
def test_bucketed_eval_av2_with_ground(
    ...
    assert len(argo_dataset_gt_with_ground) == len(argo_dataset_pseudo_with_ground), (
        f"Ground truth and pseudo datasets must have the same number of samples. "
        f"Found {len(argo_dataset_gt_with_ground)} ground truth samples and "
        f"{len(argo_dataset_pseudo_with_ground)} pseudo samples."
    )

    evaluator = argo_dataset_gt_with_ground.evaluator()

    # Iterate over both datasets, treating the pseudo dataset as the "prediction"
    # and the ground truth dataset as the "target"
    for (_, est_gt), (_, target_gt) in zip(
        argo_dataset_pseudo_with_ground, argo_dataset_gt_with_ground
    ):
...
def test_bucketed_eval_av2_no_ground(
    ...
    assert len(argo_dataset_gt_no_ground) == len(argo_dataset_pseudo_no_ground), (
        f"Ground truth and pseudo datasets must have the same number of samples. "
        f"Found {len(argo_dataset_gt_no_ground)} ground truth samples and "
        f"{len(argo_dataset_pseudo_no_ground)} pseudo samples."
    )

    evaluator = argo_dataset_gt_no_ground.evaluator()

    # Iterate over both datasets, treating the pseudo dataset as the "prediction"
    # and the ground truth dataset as the "target"
    for (_, est_gt), (_, target_gt) in zip(
        argo_dataset_pseudo_no_ground, argo_dataset_gt_no_ground
    ):
        est_gt: GroundTruthPointFlow
        target_gt: GroundTruthPointFlow
        assert all(est_gt.trajectory_timestamps == target_gt.trajectory_timestamps), (
