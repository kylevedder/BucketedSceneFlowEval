...
parser.add_argument("output_directory", type=Path, help="Path to output directory.")
parser.add_argument(
    "--cells_per_meter",
    type=int,
    default=10.0 / 3.0,
    help="Cells per meter for discritization. Default is Argoverse 2 Sensor dataset default is 30cm (10/3 cells per meter)",
)
parser.add_argument(
    "--num_neighbors",
    type=int,
...
assert args.waymo_directory.is_dir(), f"{args.waymo_directory} is not a directory"
...
def render_heightmap(points, cells_per_meter, num_neighbors):
    ...
    grid = avg_neighbor_z_values.reshape(pts_square_shape[0], pts_square_shape[1])

    return grid, grid_min_global_frame


def save_grid_global_offset(
    file_path: Path, grid, grid_min_global_frame, cells_per_meter, verbose=False
):
    se2 = {
...
Parallel(num_processes)(delayed(process_record)(record) for record in work_queue)
