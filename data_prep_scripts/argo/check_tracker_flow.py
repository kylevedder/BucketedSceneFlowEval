...
def compute_sceneflow(
    ...
    if len(cuboids) != 2:
        print(
            f"Length of id_cuboid_map = {len(cuboids)}, failing log_id: {log_id}, timestamps: {timestamps}"
        )
    return None
    # assert (
    #     len(cuboids) == 2
...
def process_log(
    ...
    for ts0, ts1 in iter_bar:
        compute_sceneflow(dataset, track_data, log_id, (ts0, ts1), confidence_threshold)
        # df = pd.DataFrame(
        #     {
